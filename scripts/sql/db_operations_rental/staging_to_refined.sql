...
LEFT JOIN   staging.rental_{{ params.region_name }} STG
ON STG.ID = TGT.ID
SET
    TGT.ID                  = STG.ID,
...
DROP TEMPORARY TABLE IF EXISTS staging.rental_staging_to_refined_new_{{ params.region_name }};
CREATE TEMPORARY TABLE staging.rental_staging_to_refined_new_{{ params.region_name }}
SELECT 
    STG.*, 
...
SELECT
    ...
    '{{ dag.timezone.convert(execution_date).strftime("%Y-%m-%d %H:%M:%S") }}',
    TRUE
FROM staging.rental_staging_to_refined_new_{{ params.region_name }} STG
;