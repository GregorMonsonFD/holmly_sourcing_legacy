...
for region in config.get('imports'):
    ...
    globals()[f'rightmove-scrape-rental-{region_name}'] = DAG(
        dag_id=f'rightmove-scrape-rental-{region_name}',
        default_args=args,
    ...
    with globals()[f'rightmove-scrape-rental-{region_name}']:
        ...
        sftp_upload_to_db = SFTPOperator(
            ...
            remote_filepath="/var/lib/mysql-files/rental_data_{{ params.rightmove_region }}_{{ ds }}.csv",
            operation="put",
        ...
        delete_csv_remote = SSHOperator(
            ...
            command=f'rm /var/lib/mysql-files/rental_data_{ rightmove_region }_{ ds }.csv',
        )
